...
export default function EditProfileScreen({ profile, onSave, onBack }) {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 999, background: '#0A0A0A', overflowY: 'auto', padding: '24px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '32px' }}>
        <button onClick={onBack} style={{ background: 'none', border: 'none', color: 'white', fontSize: '20px', cursor: 'pointer' }}>←</button>
        <h2 style={{ fontSize: '20px', fontWeight: '800' }}>Modifier le profil</h2>
      </div>
      {[['Nom', username, setUsername], ['Handle', handle, setHandle], ['Role', role, setRole], ['Pitch', bio, setBio], ['Zone', zone, setZone], ['Styles', styles, setStyles]].map(([label, val, set]) => (
        <div key={label} style={{ marginBottom: '16px' }}>
          ...
          <input value={val} onChange={e => set(e.target.value)} style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.06)', color: 'white', fontSize: '14px' }} />
        </div>
      ))}
      {error && <p style={{ color: '#FF4D4D', fontSize: '13px', marginBottom: '16px' }}>{error}</p>}
      <button onClick={handleSave} disabled={loading} style={{ width: '100%', padding: '14px', borderRadius: '24px', border: 'none', background: 'white', color: 'black', fontSize: '14px', fontWeight: '700', cursor: 'pointer' }}>
