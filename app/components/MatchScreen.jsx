...
export default function MatchScreen({ theme, setScreen }) {
  ...
  async function loadCollabs(userId) {
    ...
    if (recv && recv.length > 0) {
      ...
      const { data: senderProfiles } = await supabase.from('profiles').select('user_id, username, handle, role').in('user_id', senderIds);
    ...
    if (snt && snt.length > 0) {
      ...
      const { data: receiverProfiles } = await supabase.from('profiles').select('user_id, username, handle, role').in('user_id', receiverIds);
      setSent(snt.map(c => ({ ...c, receiverProfile: receiverProfiles?.find(p => p.user_id === c.receiver_id) })));
    } else setSent([]);
  }

  async function loadOffers(userId) {
    // Toutes les offres ouvertes sauf les miennes
        ...
        {tab === 'offres' && (
          <>
            ...
            <button
              ...
              style={{
                width: '100%', padding: '14px', borderRadius: '14px', marginBottom: '20px',
                border: `1.5px dashed ${darkMode ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.2)'}`,
                background: 'transparent', color: theme?.color,
            ...
            {showNewOffer && (
              <div style={{ background: card, borderRadius: '16px', padding: '16px', marginBottom: '20px', border: `1px solid ${cardBorder}` }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '10px' }}>
                  {ROLE_OPTIONS.map(r => (
                    <button key={r.id} onClick={() => setOfferRole(offerRole === r.id ? '' : r.id)} style={{
                      padding: '6px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: '700', cursor: 'pointer',
                      border: `1px solid ${offerRole === r.id ? theme?.color : (darkMode ? 'rgba(255,255,255,0.15)' : 'rgba(0,0,0,0.15)')}`,
                      background: offerRole === r.id ? theme?.color : 'transparent',
                      color: offerRole === r.id ? theme?.bg : subText,
                ...
                {offers.map(o => (
                  <div key={o.id} style={{ background: card, border: `1px solid ${cardBorder}`, borderRadius: '16px', padding: '16px', marginBottom: '12px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '12px' }}>
                      {o.role_needed && (
                        <span style={{ fontSize: '11px', color: theme?.color, border: `1px solid ${cardBorder}`, borderRadius: '20px', padding: '3px 10px', fontWeight: '700' }}>
                          {ROLE_OPTIONS.find(r => r.id === o.role_needed)?.icon} {o.role_needed}
                        </span>
                      )}
                    ...
                    <button
                      onClick={async () => {
                        ...
                        if (u) {
                          await supabase.from('collabs').insert({ sender_id: u.id, receiver_id: o.user_id, message: `Candidature pour : ${o.title}`, status: 'pending' });
              ...
              <>
                ...
                {received.map(c => (
                  <div key={c.id} style={{ background: card, border: `1px solid ${cardBorder}`, borderRadius: '14px', padding: '16px', marginBottom: '10px' }}>
                    ...
                    <p style={{ color: subText, fontSize: '12px', marginBottom: c.status === 'pending' || c.status === 'accepted' ? '12px' : '0' }}>{c.message || 'Pas de message'}</p>
                    {c.status === 'pending' && (
